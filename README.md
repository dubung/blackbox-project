...
**범위(Scope)**  
- 본 프로젝트는 `Main App ↔ Child Process` 구조로 동작하며, **전체 구조 설계/프로세스 분리 및 연동 설계는 팀(조장) 담당**입니다.  
- 저는 **Child Process 내부에서 PETRv2 추론 + 영상 처리/시각화 파이프라인 구현**을 전담했습니다.

**구현한 내용(Child Process 내부 작업)**  
- **Petrv2 추론 런타임 구성**
  - Child Process 환경에서 PETRv2 모델 실행(로딩/초기화/추론 실행 흐름)
  - 입력 데이터 포맷 정합 및 추론 호출 루틴 구현
- **후처리 및 시각화**
  - 추론 결과(예: 3D box 등) 후처리 → **Overlay / BEV 시각화** 변환
  - 영상 프레임과 결합해 디스플레이 출력(레이어 구성, 좌표계 반영)
- **안정화/운영성 보강**
  - 프레임 지연/드롭 상황 처리(큐 관리, 스킵/드롭 정책)
  - 예외 처리 및 로깅(모델 로딩 실패, 입력 누락, 추론 오류 등)

**구현 목표 및 이유**  
- 연산량이 큰 추론/영상처리 파트를 Child Process 내부에서 안정적으로 동작시키고,  
  **즉시 시각화 가능한 결과물**을 제공해 디버깅/검증 시간을 단축했습니다.  
- 또한 장애 상황에서도 원인 파악이 가능하도록 **로그/예외처리 기반의 운영 가능한 형태**로 정리했습니다.
